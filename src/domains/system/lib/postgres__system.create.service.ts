...
export async function createSystem__postgres(data: { system: SystemSpecs }) {
  try {
    const s = data.system;
    ...
    if (typeof s.ownerEmail === 'string') dataToCreate.ownerEmail = s.ownerEmail;
    if (typeof s.controllerName === 'string') dataToCreate.controllerName = s.controllerName;
